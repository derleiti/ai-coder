...
- No args = setup wizard + agent REPL (setup.py)
- Agent loop: agent.py — local_exec runs locally, MCP tools read-only
- UI primitives: ui.py (Braille spinners, panels, ANSI colors)
- Config: ~/.config/ai-coder/session.json + state.json
- Build: .venv/bin/pyinstaller aicoder.spec → dist/aicoder (7.8MB standalone)
