## v0.8.3 — Backend Performance Fixes (2026-04-14)

### Performance (Critical)
- **Connection Pooling**: urllib3 PoolManager with keep-alive — eliminates TCP+TLS handshake per API call (~200-500ms saved per call in agent loops)
- **Tool Cache**: MCP tools/list results cached for 5min with TTL — eliminates 2-20s blocking on every agent start
- **Retry Logic**: Automatic retry with exponential backoff on 5xx/timeout errors in all backend calls (client, MCP tools)

### Bug Fixes
- **Swarm Thread Joins**: Fixed sequential t1.join(90) + t2.join(90) → now parallel with shared deadline (max 90s total, was up to 180s)
- **Redundant Sessions**: swarm_runner no longer creates new TriForceClient per call — reuses caller's client (saves disk reads + TLS)
- **Silent Model Errors**: list_models() now reports TokenExpired/ClientError on stderr instead of silently returning empty list
- **Chat History Explosion**: Multi-turn chat now caps each history response at 2000 chars to prevent context window overflow
- **Fallback Tool Warning**: Degraded agent mode (when tools/list fails) now shows prominent multi-line warning with troubleshooting hint

### Files Changed
- `client.py` — Connection pooling (_get_pool), retry (_request/_do_request), error reporting
- `executor.py` — Tool cache (load_tools), MCP tool retry (run_mcp_tool)
- `swarm_runner.py` — Parallel thread joins, client passthrough, time import
- `cli.py` — Chat history size limit
