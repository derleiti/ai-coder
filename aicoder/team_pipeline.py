...
def test_change_evidence(delta: dict[str, Any]) -> dict[str, Any]:
    """Classify merged/candidate changes so behavior changes cannot silently ship without test edits."""
    ...
    source_paths = [
        ...
        if not _is_test_path(path) and Path(path).suffix.lower() in _SOURCE_SUFFIXES
    ]
    return {
        "source_paths": source_paths,
        "test_paths": test_paths,
        "behavior_change": bool(source_paths),
        "tests_changed": bool(test_paths),
